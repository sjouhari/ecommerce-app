    ...
    public CommandLineRunner initDatabase(ProfilRepository profilRepository, FeatureRepository featureRepository, UserRepository userRepository, PasswordEncoder passwordEncoder) {
        return args -> {
           ...
           if(userRepository.count() == 0) {
               User admin = new User(null, "Admin", "Admin", "admin@gmail.com", passwordEncoder.encode(ADMIN_PASSWORD), true, true, 0, LocalDateTime.now(), null, new ArrayList<>(profilRepository.findAll()), false, "");
